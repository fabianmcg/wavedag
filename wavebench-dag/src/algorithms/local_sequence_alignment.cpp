...
int LSA::r() { return rows; }
int LSA::c() { return cols; }
 ...
 * ---*/
void LSA::run() { create_score_matrix(rows, cols); }
...
#pragma omp dag coarsening(BLOCK, bx, by)
  for (int x = 1; x < rows; x++)
    for (int y = 1; y < cols; y++) {
#pragma omp dag task depend(                                                   \
        {(x + 1) * cols + y + 1, ((x + 1) < rows) && ((y + 1) < cols)},        \
            {(x + 1) * cols + y, (x + 1) < rows},                              \
            {x * cols + y + 1, (y + 1) < cols})
