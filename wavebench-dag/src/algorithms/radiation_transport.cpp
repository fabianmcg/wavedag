                          ...
                          NU * (ie +
                                NE * (ix + dims.ncell_x *
                                               (iy + dims.ncell_y *
                                                         (iz + dims.ncell_z *
                                                                   (0))))))] =
                  (real)(Quantities_scalefactor_space(ix, iy, iz) *
                         (real)Quantities_scalefactor_energy(ie) * iu) +
                  (real)(im * iu);
            }
  ...
  for (int octant = 0; octant < NOCTANT; ++octant)
    for (int im = 0; im < NM; ++im)
      for (int ia = 0; ia < NA; ++ia) {
        m_from_a[im + NM * (ia + NA * (octant + NOCTANT * (0)))] =
            (real)(im + 1) + (1.0 / (octant + 1));
      }
}
        ...
        for (ix = 0; ix < dim_x; ++ix)
          ...
          for (ie = 0; ie < NE; ++ie)
            for (iu = 0; iu < NU; ++iu)
              for (ia = 0; ia < NA; ++ia) {
                iz = -1;
                int scalefactor_space =
                    Quantities_scalefactor_space(ix, iy, iz);
...
void RT::run() {
  ...
  auto coordinates = [](int o, int x, int y, int z, int dim_z, int dim_y,
                        int dim_x) {
    return ((o * dim_z + z) * dim_y + y) * dim_x + x;
  };
...
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix, iy, iz + 1, dim_z, dim_y, dim_x)),           \
             ((iz + 1) < dim_z)})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix, iy + 1, iz, dim_z, dim_y, dim_x)),           \
             ((iy + 1) < dim_y)})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix + 1, iy, iz, dim_z, dim_y, dim_x)),           \
             ((ix + 1) < dim_x)})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix + 1, iy + 1, iz, dim_z, dim_y, dim_x)),       \
             (((ix + 1) < dim_x) && ((iy + 1) < dim_y))})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix + 1, iy, iz + 1, dim_z, dim_y, dim_x)),       \
             (((ix + 1) < dim_x) && ((iz + 1) < dim_z))})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix, iy + 1, iz + 1, dim_z, dim_y, dim_x)),       \
             (((iz + 1) < dim_z) && ((iy + 1) < dim_y))})
#pragma omp dag depend(                                                        \
        {(coordinates(octant, ix + 1, iy + 1, iz + 1, dim_z, dim_y, dim_x)),   \
             ((((iz + 1) < dim_z) && ((iy + 1) < dim_y)) &&                    \
                  ((ix + 1) < dim_x))})
#pragma omp dag depend(                                                        \
        {(coordinates(octant + 1, 0, 0, 0, dim_z, dim_y, dim_x)),              \
             (((((octant + 1) < NOCTANT) && (ix >= (dim_x - 1))) &&            \
                   (iy >= (dim_y - 1)))) &&                                    \
                     (iz >= (dim_z - 1))})
#pragma omp dag task
...
void RT::solve(int ix, int iy, int iz, int octant) {
...
---*/
  ...
  const real scalefactor_space_x_r =
      ((real)1) / Quantities_scalefactor_space(ix - 1, iy, iz);
  const real scalefactor_space_y_r =
      ((real)1) / Quantities_scalefactor_space(ix, iy - 1, iz);
  const real scalefactor_space_z_r =
      ((real)1) / Quantities_scalefactor_space(ix, iy, iz - 1);
  ...
  for (iu = 0; iu < NU; ++iu) {
    ...
    const real result =
        (real)1.0 /
        (local[local_index] * scalefactor_space_r +
         (
             /*--- ref_facexy inline ---*/
             facexy[ia +
                    NA * (iu +
    ...
    /*--- ref_facexy inline ---*/
    facexy[ia +
           NA * (iu + NU * (ie + NE * (ix + dims.ncell_x *
                                                (iy + dims.ncell_y *
                                                          (octant +
                                                           NOCTANT * (0))))))] =
        result_scaled;
    ...
    /*--- ref_faceyz inline ---*/
    faceyz[ia +
           NA * (iu + NU * (ie + NE * (iy + dims.ncell_y *
                                                (iz + dims.ncell_z *
                                                          (octant +
                                                           NOCTANT * (0))))))] =
        result_scaled;
...
real RT::Quantities_init_face(int ia, int ie, int iu, int scalefactor_space,
                              int octant) {
...
int RT::Quantities_scalefactor_space(int ix, int iy, int iz) {
  ...
  scalefactor_space =
      ((scalefactor_space + (ix + 3 * iy + 7 * iz + 2)) * 8121 + 28411) %
      134456;
